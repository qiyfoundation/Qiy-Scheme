...
* The [Individual](Definitions.md#individual) subscribes to a service.
* The [Relying Party](Definitions.md#relying-party) asks the [Individual](Definitions.md#individual) for the data it needs to provide the service.
* The [Individual](Definitions.md#individual) retrieves the data from a [Data Provider](Definitions.md#data-provider).
* The [Individual](Definitions.md#individual) provides the data to the [Relying Party](Definitions.md#relying-party).
...
The Data Reuse scenario shows that the data is transferred to the [Relying Party](Definitions.md#relying-party) by choice of the [Individual](Definitions.md#individual).
This breaks the chain of responsibility for the [Data Provider](Definitions.md#data-provider); the responsibility for correct processing of the data does not extend to any processing that takes place after the handover to the [Individual](Definitions.md#individual). 
...
* The [Individual](Definitions.md#individual) can securily exchange data and/or messages with another person or organization ([Qiy User](Definitions.md#qiy-user)) via Qiy, using connections, see [4 The User Layer](#4-the-user-layer).
...
* When an [Individual](Definitions.md#individual) connects with a [Qiy User](Definitions.md#qiy-user) that is providing a [Service](Definitions.md#service) via Qiy ([Provider](Definitions.md#provider)), the [Individual](Definitions.md#individual) is provided with the identity of the latter, but not the other way around.
...
* The [Individual](Definitions.md#individual) controls what data he shares with what [Provider](Definitions.md#provider) ([Relying Party](Definitions.md#relying-party)) and under what terms using proveable [Consents](Definitions.md#consent).
* The [Data Provider](Definitions.md#data-provider) knows what data is obtained by (or in name of) an [Individual](Definitions.md#individual), but he does not know whether the data is shared and if so, with what [Relying Parties](Definitions.md#relying-party).
* [Relying Parties](Definitions.md#relying-party) however know the [Data Provider](Definitions.md#data-provider) of shared data and can use this information to assess the trustworthiness of the data and/or to verify the validity of the data.
...
* Access to Qiy, data exchange via Qiy, consent services and potentially [Qiy Nodes](Definitions.md#qiy-node) are provided by [Access Providers](Definitions.md#access-provider).
...
* [Application Providers](Definitions.md#application-provider) can only develop and produce [Qiy Application](Definitions.md#qiy-application)-services and/or software with a [Licence Agreement Application Provider](Definitions.md#licence-agreement-application-provider).
...
* Applications exchange data and/or messages via Qiy using open standards of the [Qiy Scheme](Definitions.md#qiy-scheme) ([Qiy Open Standard](Definitions.md#qiy-open-standard)).
* Applications exchange self-describing data and/or messages using [Data Descriptions](Definitions.md#data-description) which are available to all concerned parties (via the [Service Library](Definitions.md#service-library)).
...
Two [Qiy Users](Definitions.md#qiy-user) can connect via Qiy by creating a connection between their [Qiy Nodes](Definitions.md#qiy-node) ([Connection](Definitions.md#connection)).
...
* The [Proposer](Definitions.md#proposer) uses a [Qiy Application](Definitions.md#qiy-application) to generate a token (see [4.4.1 Generate token](#441-generate-token)) and to compose a [Connect Proposal](Definitions.md#connect-proposal).
...
As stated before, when a [Connection](Definitions.md#connection) is established, the identity of the [Qiy User](Definitions.md#qiy-user) is provided to the other one if the Qiy User is a [Provider](Definitions.md#provider). 
...
A [Proposer](Definitions.md#proposer) can create a token using a [Qiy Application](Definitions.md#qiy-application) and the following details:
...
* Budget: The number of times that the token can be used to create a [Connection](Definitions.md#connection).
...
#### 4.4.2.1 The web
[Qiy Users](Definitions.md#qiy-user) can connect by transfering a token as a query parameter in a website address:
...
#### 4.4.2.2 Print
[Qiy Users](Definitions.md#qiy-user) can convert the token to a QR Code and use various 'Print'-media to connect:
...
In order to be able to offer his services via Qiy, a [Relying Party](Definitions.md#relying-party) has met the following preconditions:
* The [Relying Party](Definitions.md#relying-party) has acquired access to Qiy with the help of an Access Provider.
...
* The [Service Library](Definitions.md#service-library) contains the [Service Catalogue](Definitions.md#service-catalogue) of the [Relying Party](Definitions.md#relying-party) defining the provided services.
* The [Service Library](Definitions.md#service-library) contains [Service Descriptions](Definitions.md#service-description) for all the provided services, which also includes the terms of use, especially with regard to Personal Data.
...
* The [Data Provider](Definitions.md#data-provider) has acquired access to Qiy with the help of an Access Provider.
...
* A [Service Endpoint](Definitions.md#service-endpoint) is available to access the data.
* The [Service Library](Definitions.md#service-library) contains the [Service Endpoint API](Definitions.md#service-endpoint-api) which describes how the data can be obtained.
* The [Service Library](Definitions.md#service-library) contains [Data Descriptions](Definitions.md#data-description) for the available data.
* The [Service Library](Definitions.md#service-library) contains the [Service Catalogue](Definitions.md#service-catalogue) of the [Data Provider](Definitions.md#data-provider) defining the provided data services and the related endpoints.
* The [Service Library](Definitions.md#service-library) contains [Service Descriptions](Definitions.md#service-description) for the provided data services.
...
When an [Individual](Definitions.md#individual) subscribes to a service, the subscription is registered by the [Qiy Application](Definitions.md#qiy-application), so:
* The subscribed service is recorded using the [Service Portfolio](Definitions.md#service-portfolio) of the [Individual](Definitions.md#individual).
* The record shows:
	* the start datetime of the subscription.
	* the [Provider](Definitions.md#provider) of the service (the [Relying Party](Definitions.md#relying-party)).
	* what service is provided (using the [Service Library](Definitions.md#service-library)).
...
When all related conditions are met, a request for data from a [Relying Party](Definitions.md#relying-party) is processed as follows:
...
* Using the API, requests are created and used to obtain the data from the [Service Endpoints](Definitions.md#service-endpoint).
* The received data is forwarded to the [Relying Party](Definitions.md#relying-party).
...
The [Connect Proposal](Definitions.md#connect-proposal) can be used by websites as a user friendly login alternative:

![Login](./images/example-application--login.PNG)

### 4.11.4 Share Medical Data

The [Connect Proposal](Definitions.md#connect-proposal) can be used to anonymously share medical data for a visit at a medical facility:
...
(Diagram source code: [10.2.2.1 Relying Party: Request consent](#10221-relying-party-request-consent))
...
A [Qiy Application](Definitions.md#qiy-application) can present an [Individual](Definitions.md#individual) a list of suitable [Data Providers](Definitions.md#data-provider) (or in general [Providers](Definitions.md#provider)) that can produce some requested data (or services) as follows:
...
This goes as follows:
* A [Qiy Application](Definitions.md#qiy-application) requests a reference for the data ([Data Reference](Definitions.md#data-reference)).
* The [Qiy Application](Definitions.md#qiy-application) receives a [Data Reference](Definitions.md#data-reference).
* The [Qiy Application](Definitions.md#qiy-application) uses the [Data Reference](Definitions.md#data-reference) to acquire the data.

### 5.5.1 Service by Reference
In Qiy providing data is viewed as a service and requesting data as an operation of this service, so the 'data by reference'-pattern is implemented as using a [Service by Reference](Definitions.md#service-by-reference)-pattern:
...
* The [Qiy Application](Definitions.md#qiy-application) can call for the execution of an operation by submitting the [Operation Reference](Definitions.md#operation-reference) in an [Operation Execute Request](Definitions.md#operation-execute-request).

### 5.5.2 Request data reference
The [Qiy Application](Definitions.md#qiy-application) of a [Relying Party](Definitions.md#relying-party) can request an [Individual](Definitions.md#individual) for a data reference as follows:
...
(Diagram source code: [10.2.3.1 Relying Party: Request data](#10231-relying-party-request-data))

### 5.5.3 Create reference
A [Qiy Application](Definitions.md#qiy-application) can create an [Operation Reference](Definitions.md#operation-reference) using a specification of the operation ([Operation Specification](Definitions.md#operation-specification)).
This goes as follows:
...
### 5.5.4 Request data
The [Qiy Application](Definitions.md#qiy-application) of a [Relying Party](Definitions.md#relying-party) can obtain data using a [Data Reference](Definitions.md#data-reference) / [Operation Reference](Definitions.md#operation-reference). 
This goes as follows:
...
* The [Qiy Node](Definitions.md#qiy-node) returns the requested data.
...
### 5.5.5 Provide data
The [Data Provider](Definitions.md#data-provider) produces the data using his [Service Endpoint](Definitions.md#service-endpoint).
...
## 6.1 Access Provider
The services of this layer can be provided by an [Access Provider](Definitions.md#access-provider):
...
### 6.1.1 Portability
An [Access Provider](Definitions.md#access-provider) can offer [Qiy Node](Definitions.md#qiy-node)-services to [Qiy Users](Definitions.md#qiy-user), but must enable [Qiy Users](Definitions.md#qiy-user) to easily transfer the services to a different [Access Provider](Definitions.md#access-provider).
...
* ... has its own [Transporter](Definitions.md#transporter) which ensures secure transport of messages and/or data via Qiy.
...
* How [Qiy Nodes](Definitions.md#qiy-node) create [Connections](Definitions.md#connection) and use them to exchange data, messages or to provide/consume services.
...
In this case, the [Qiy Node](Definitions.md#qiy-node) will be deleted including persisted data, [Connections](Definitions.md#connection) and the linked [Transporter](Definitions.md#transporter).
...
### 6.3.2 Connect Token
A [Connect Token](Definitions.md#connect-token) is a token which can be used by a [Qiy Application](Definitions.md#qiy-application) to create a [Connection](Definitions.md#connection).
It consists of:
* a temporary secret
...
However, care must be taken that the created token is unique, especially so for the created [Transport Connect Token](Definitions.md#transport-connect-token).
...
The [Connection](Definitions.md#connection) will be deleted completely, including any persisted data and/or messages and underlying [Paths](Definitions.md#path).
...
### 7.1.1 Portability
The [Qiy Scheme](Definitions.md#qiy-scheme) prescribes that one can easily change to a different [Access Provider](Definitions.md#access-provider) for these services.
...
## 8.1 Access Provider
The services of this layer are only provided by an [Access Provider](Definitions.md#access-provider).

### 8.1.1 Portability
The [Qiy Scheme](Definitions.md#qiy-scheme) prescribes that one can easily switch [Access Provider](Definitions.md#access-provider) for these services.

## 8.2 Transporter
A [Transporter](Definitions.md#transporter) is a [Technology Service](Definitions.md#technology-service) which allows the secure transmission of messages and/or data.
...
In this case, the [Transporter](Definitions.md#transporter) will be deleted including any persisted data and [Routes](Definitions.md#route).

## 8.8 Path
A [Path](Definitions.md#path) is a logical connection between two [Transporters](Definitions.md#transporter) that can be used to exchange [Transport Messages](Definitions.md#transport-message).
...
The [Path](Definitions.md#path) will be deleted including any persisted data and/or messages.
...
* To safely transport messages between [Carriers](Definitions.md#carrier).
