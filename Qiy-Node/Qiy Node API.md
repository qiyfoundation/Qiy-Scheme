...
This [Event Callbacks Endpoint](#event-callbacks-endpoint)-request can be used to define the addresses of the [Event Callback Endpoints], see [Set event callback endpoints request].
...
This server-sent [event][Events] is generated by a Qiy Node of a Relying Party when a source is set or added for a feed, see [Request for feed], [Add feed source] or [Set feed source].
